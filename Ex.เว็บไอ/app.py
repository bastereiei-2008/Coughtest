...
# ==========================================
# PAGE 2: Audio Recording (Minimalist Red Button)
# ==========================================
elif st.session_state.page == 2:
    # --- CSS ขั้นสูง: ปรับแต่งเฉพาะปุ่มไมค์ ---
    st.markdown("""
    <style>
        /* 1. ลบกรอบสี่เหลี่ยมและพื้นหลังของทั้ง Widget ทิ้ง */
        div[data-testid="stAudioInput"] {
            ...
            width: fit-content !important; /* ให้กว้างเท่าที่จำเป็น */
        }

        /* 2. ปรับแต่งตัว "ปุ่มกด" (Start Button) ข้างในให้เป็นวงกลมสีแดง */
        div[data-testid="stAudioInput"] > button {
            background-color: #FF4B4B !important; /* สีแดง */
            width: 150px !important;       /* กว้าง 150px */
            height: 150px !important;      /* สูง 150px */
            border-radius: 50% !important; /* ทำเป็นวงกลม */
            border: 4px solid #FFEBEE !important; /* ขอบสีขาวจางๆ */
            display: flex !important;
            justify-content: center !important;
            align-items: center !important;
            margin: 0 auto !important;
            box-shadow: 0 4px 15px rgba(255, 75, 75, 0.4) !important; /* เงา */
        ...
        /* 3. ปรับขนาดไอคอนไมค์ข้างในให้ใหญ่ขึ้น */
        div[data-testid="stAudioInput"] > button svg {
            width: 60px !important;
            height: 60px !important;
            fill: white !important; /* ไอคอนสีขาว */
        }

        /* 4. ซ่อนตัว Waveform (คลื่นเสียง) ให้มากที่สุดเท่าที่ทำได้ */
        /* หมายเหตุ: Streamlit อาจจะยังโชว์เส้นขีดเล็กๆ ตอนอัด แต่เราทำให้มันกลมกลืน */
        div[data-testid="stAudioInput"] canvas {
            display: none !important; /* พยายามซ่อนคลื่นเสียง */
        }
    ...
    # --- ปุ่มอัดเสียง (จะแสดงผลเป็นปุ่มแดงกลมๆ ปุ่มเดียว) ---
    audio = st.audio_input("Record") 
    
    # --- Logic การทำงาน ---
    if audio:
        # เมื่ออัดเสร็จแล้ว:
        # 1. เราจะไม่สั่ง st.audio(audio) เพื่อไม่ให้มีเครื่องเล่นโผล่มา
        # 2. เราจะไม่แสดงข้อความสีเขียว
        # 3. แสดงปุ่ม "ไปต่อ" ทันที
        ...
        with c2:
            # เปลี่ยนข้อความเป็น "บันทึกเสร็จสิ้น! กดเพื่อวิเคราะห์"
            if st.button("✅ บันทึกเสร็จสิ้น! กดเพื่อวิเคราะห์ต่อ ➔", type="primary"):
                next_page()
    
    else:
        # ถ้ายังไม่อัด ก็เว้นที่ว่างไว้เฉยๆ
        st.write("<br><br>", unsafe_allow_html=True)

    st.write("")
    # ปุ่มย้อนกลับแบบเรียบๆ
